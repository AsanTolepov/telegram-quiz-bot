...
# --- SOZLAMALAR ---
BOT_TOKEN = "8598805465:AAFdctg9nmECo_uJlSi4xqfEl6bosuOmL9M" # <-- Tokenni shu yerga qo'ying
...
# --- YAKUNIY SAQLASH VA NOM O'ZGARTIRISH (MUHIM) ---
async def finish_creation(message: types.Message, state: FSMContext, parts_count: int):
    ...
    for i in range(parts_count):
        ...
        # --- MUHIM: NOMNI O'ZGARTIRISH (1-25) ---
        # Raqamlarni to'g'irlash (indeks 0 dan boshlanadi, bizga 1 dan kerak)
        ...
        # Har bir bo'lak saqlangach, darhol bazaga yozamiz
        save_db(SETUP_DATA)
...
async def inline_handler(query: types.InlineQuery):
    ...
    # Bazani har safar yangilab olamiz (xatolik oldini olish uchun)
    global SETUP_DATA
    SETUP_DATA = load_db()

    items = []
    # Agar aniq ID kelsa (Button bosilganda)
    if q in SETUP_DATA:
        items.append((q, SETUP_DATA[q]))
    else:
        # Oddiy qidiruv (Oxirgi 20 ta)
...
async def run_quiz_logic(message: types.Message, quiz_id: str):
    ...
    if quiz_id not in SETUP_DATA:
        await message.answer("⚠️ <b>Xatolik!</b> Bu test bazada topilmadi.\nEhtimol o'chirilgan yoki ID xato.", parse_mode="HTML")
        return
        
    chat_id = message.chat.id
    if chat_id in ACTIVE_GAMES and ACTIVE_GAMES[chat_id]['is_running']:
        await message.answer("⚠️ Bu yerda allaqachon test ketmoqda. To'xtatish uchun /stop")
        return
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🚀 BOSHLASH", callback_data="start_game")]])
