...
# Dastur boshlanganda bazani yuklab olamiz
SETUP_DATA = load_db()
...
def parse_quiz_text(text):
    questions = []
    # Raqamlarni yangi qatorga tushirish (1. Savol...)
    ...
    for block in raw_blocks:
        if not block.strip():
            continue
        # Variantlarni ajratish (a) b) c)...)
        parts = re.split(r'\n\s*[a-dA-Dа-яА-Я][\)\.]\s*', block)
        if len(parts) < 2:
            parts = re.split(r'\s+[a-dA-Dа-яА-Я][\)\.]\s+', block)
        if len(parts) >= 2:
            q_text = parts[0].strip()
            # Juda uzun savollarni qisqartirish
            if len(q_text) > 250:
                q_text = q_text[:247] + "..."
            raw_options = [p.strip() for p in parts[1:] if p.strip()]
            options = [opt[:99] for opt in raw_options]  # Telegram limiti 100 belgi
            if len(options) >= 2:
...
async def cmd_start(message: types.Message):
    # Agar start orqali test boshlansa
    args = message.text.split()
    if len(args) > 1 and args[1].startswith("run_"):
        await run_quiz_logic(message, args[1].replace("run_", ""))
        return
    ...
    await message.answer("👋 Test Botga xush kelibsiz!\nTanlang 👇", parse_mode="HTML", reply_markup=kb)
...
async def show_my_tests(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        "📂 <b>O'chirish uchun testni tanlang:</b>",
        parse_mode="HTML",
...
async def set_name(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        "📥 <b>Savollarni yuboring:</b>\n<i>(Word yoki Telegramdan nusxalab tashlasangiz bo'ladi)</i>",
        parse_mode="HTML"
    )
...
async def collect_text(message: types.Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="✅ YUKLASH TUGADI", callback_data="text_done")]]
    )
    await message.answer(
        f"⏳ Qabul qilindi (+{len(message.text)} belgi).\n"
        "Yana bo'lsa tashlang, bo'lmasa tugmani bosing 👇",
        reply_markup=kb
    )
...
async def process_text_done(callback: types.CallbackQuery, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Hammasi A", callback_data="ans_all_a"),
         InlineKeyboardButton(text="Hammasi B", callback_data="ans_all_b")],
        [InlineKeyboardButton(text="Hammasi C", callback_data="ans_all_c"),
         InlineKeyboardButton(text="Hammasi D", callback_data="ans_all_d")]
    ])
...
async def save_answers(message: types.Message, state: FSMContext, answers):
    ...
    await message.answer(
        "⏱ <b>Har bir savolga necha soniya vaqt berasiz?</b>\n"
        "(Faqat raqam yozing, masalan: 15)",
        parse_mode="HTML"
    )
...
async def set_shuffle_mode(callback: types.CallbackQuery, state: FSMContext):
    ...
    if q_count >= 10:
        kb_list.append([InlineKeyboardButton(text="✂️ 2 bo'lak", callback_data="split_2")])
    if q_count >= 20:
        kb_list.append([InlineKeyboardButton(text="✂️ 3 bo'lak", callback_data="split_3")])
    if q_count >= 30:
        kb_list.append([InlineKeyboardButton(text="✂️ 4 bo'lak", callback_data="split_4")])
...
async def process_split_choice(callback: types.CallbackQuery, state: FSMContext):
    ...
    if choice == "manual":
        await callback.message.edit_text(
            "🔢 <b>Necha bo'lakka bo'lmoqchisiz?</b>\n(Raqam yozib yuboring)",
            parse_mode="HTML"
        )
...
async def finish_creation(message: types.Message, state: FSMContext, parts_count: int):
    ...
    await message.answer(
        f"✅ <b>TAYYOR!</b>\nTest saqlandi va {parts_count} qismga bo'lindi.",
        parse_mode="HTML"
    )
    ...
    for i in range(parts_count):
        ...
        if parts_count > 1:
            part_name = f"{data['quiz_name']} ({real_start}-{real_end})"
        else:
            part_name = data['quiz_name']
...
async def inline_handler(query: types.InlineQuery):
    ...
    for q_id, data in items:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Bu testni boshlash", url=f"https://t.me/{bot_usr}?start=run_{q_id}")],
            [InlineKeyboardButton(text="Guruhda testni boshlash", url=f"https://t.me/{bot_usr}?startgroup=run_{q_id}")],
            [InlineKeyboardButton(text="Testni ulashish", switch_inline_query=q_id)]
        ])
...
# --- O'YIN LOGIKASI ---
async def run_quiz_logic(message: types.Message, quiz_id: str):
    global SETUP_DATA
    SETUP_DATA = load_db()

    if quiz_id not in SETUP_DATA:
        await message.answer(
            "⚠️ <b>Xatolik!</b> Bu test bazada topilmadi.\n"
            "Ehtimol o'chirilgan yoki ID xato.",
            parse_mode="HTML"
        )
        return
    ...
    if chat_id in ACTIVE_GAMES and ACTIVE_GAMES[chat_id]['is_running']:
        await message.answer(
            "⚠️ Bu yerda allaqachon test ketmoqda. "
...
async def start_loop(callback: types.CallbackQuery):
    ...
    for i, q in enumerate(game['questions']):
        if not ACTIVE_GAMES.get(chat_id, {}).get('is_running'):
            break
        if game['empty_streak'] >= 3:
            await bot.send_message(
                chat_id,
                "💤 <b>Faollik yo'q. Test to'xtatildi.</b>",
                parse_mode="HTML"
            )
            break
...
async def handle_answer(ans: PollAnswer):
    if ans.poll_id in POLL_MAPPING:
        ...
        if cid in ACTIVE_GAMES:
            ...
            uid = ans.user.id
            if uid not in sc:
                sc[uid] = {"name": ans.user.full_name, "score": 0}
            if ans.option_ids[0] == data['correct_id']:
                sc[uid]["score"] += 1
...
async def main():
    # Botni fon task sifatida ishga tushiramiz
    asyncio.create_task(dp.start_polling(bot))

    # Oddiy web-server (Render portni ko'rsin)
    app = web.Application()
    app.router.add_get("/", health)

    runner = web.AppRunner(app)
    await runner.setup()
    port = int(os.getenv("PORT", 10000))  # Render shu envni beradi
